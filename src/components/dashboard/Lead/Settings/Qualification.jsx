...
export default function Qualification() {
  ...
  const [form, setForm] = useState({ id: null, qualification: "" });
  ...
  const fetchData = () => {
    setLoading(true);
    xFetch({
      path: `/services/profile/getStudyLevel`
    })
      .then((data) => {
        setData(data);
        setFiltered(data);
  ...
  const handleSave = async () => {
    const newErrors = {};
    if (!form.studyLevelName) newErrors.studyLevelName = "Qualification is required";
    setErrors(newErrors);
    if (Object.keys(newErrors).length > 0) return;

    const url = editing
      ? "/services/profile/updateStudyLevel"
      : "/services/profile/addStudyLevel";
    ...
    xFetch({
      ...
      .then(() => {
        toast.success(editing ? "Qualification updated successfully" : "Qualification added successfully");
        setShowModal(false);
        setErrors({});
  ...
  const handleDelete = async (id, studyLevelName = "") => {
    const confirmMessage = studyLevelName 
      ? `Are you sure you want to delete the qualification "${studyLevelName}"?` 
    ...
    try {
      await xFetch({
        path: "/services/profile/deleteStudyLevel",
        method: "POST",
        payload: { studyId: id },
      });
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Are you sure you want to delete ${selected.length} selected qualifications? This action cannot be undone.`)) {
      return;
    }
    ...
    for (const id of selected) {
      try {
        ...
        console.error(`Failed to delete qualification ID: ${id}`, error);
      }
    }
                    ...
                    <td className="p-2 text-center space-x-2">
                      <button
                        onClick={() => {
                          setForm({
                            studyId: row.id,
                            studyLevelName: row.studyLevelName || row.qualification || ""
                        });
                          setEditing(true);
                          setShowModal(true);
                        }}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-xl w-96 shadow-lg">
            <h3 className="text-lg mb-4">
              {editing ? "Update" : "Add"} Qualification
            </h3>

            <input
              name="studyLevelName"
              placeholder="Qualification"
              value={form.studyLevelName}
            ...
            />
            {errors.studyLevelName && (
              <p className="text-red-500 text-sm mb-2">{errors.studyLevelName}</p>
            )}
