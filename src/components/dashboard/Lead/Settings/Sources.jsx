...
export default function Sources() {
  ...
  const [form, setForm] = useState({ id: null, source: "" });
  ...
  const fetchData = () => {
    setLoading(true);
    xFetch({
      path: `/services/profile/getSources`
    })
      .then((data) => {
        setData(data);
        setFiltered(data);
    ...
    xFetch({
      ...
      .catch((error) => {
        if (error.status === 409 && error.body?.error === "duplicate") {
          toast.error(error.body.message || "This source already exists");
          setShowModal(false);
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-xl w-96 shadow-lg">
            ...
            />
            {errors.source && (
              <p className="text-red-500 text-sm mb-2">{errors.source}</p>
            )}
