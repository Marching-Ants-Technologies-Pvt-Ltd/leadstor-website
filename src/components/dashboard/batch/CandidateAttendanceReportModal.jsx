...
export default function CandidateAttendanceReportModal({ batch, onClose }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const data = await xFetch({
        path: '/services/attendance/candidateAttendanceReport',
        method: 'POST',
        payload: {
      ...
      toast.success('Candidate attendance report generated')
    } catch (err) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            ...
            <div>
              <button
                ...
                disabled={loading}
                className="w-full md:w-auto px-6 py-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded-md disabled:opacity-50 transition flex items-center justify-center gap-2"
              >
                {loading ? (
