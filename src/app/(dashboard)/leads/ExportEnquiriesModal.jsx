...
self.onmessage = async function(e) {
    ...
    try {
        ...
        while (processedCount < totalToExport) {
            ...
            // Check if we need XLSX library (only import if needed for chunking)
            if (!self.XLSX) {
            ...
            if (isFirstChunk) {
                // Initialize merged workbook with first chunk
                mergedWorkbook = chunkWorkbook;
                mergedWorksheet = chunkWorksheet;
                isFirstChunk = false;
            } else {
                // Append data from subsequent chunks
