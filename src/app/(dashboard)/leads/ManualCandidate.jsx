    ...
    <div className="relative w-full max-w-lg mx-auto bg-white rounded-2xl shadow-xl p-0 overflow-hidden animate-fadeIn pt-10">
      ...
      >
          ...
          {rows.map((row, idx) => (
            <div key={idx} className="relative bg-white border border-gray-100 rounded-2xl shadow-sm p-5 group transition-all">
              <div className="flex justify-between items-center mb-3">
                <h3 className="font-semibold text-base text-blue-800">Candidate #{idx + 1}</h3>
                {rows.length > 1 && (
                  <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-3">
                <div>
                  ...
                  <input
                    className="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-500 bg-blue-50"
                    value={row.firstName}
        ...
        <div className="mt-1 flex items-center gap-4">
          <button
            className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white px-3 py-1.5 rounded-lg flex items-center transition-all duration-200 text-xs font-medium shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
            onClick={() => setRows([...rows, { ...initialRow }])}
            type="button"
          >
            <span className="text-lg mr-1 font-light">+</span>Add Another Candidate
          </button>
          <button
            className="bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white px-4 py-1.5 rounded-lg flex items-center transition-all duration-200 text-sm font-semibold shadow-md hover:shadow-lg disabled:opacity-60"
            type="submit"
            disabled={loading}
          >
            <span className="mr-1">✈️</span>{loading ? 'Adding...' : 'Add Enquiry'}
          </button>
        </div>
      </form>
    </div>
